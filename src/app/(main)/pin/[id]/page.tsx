        ...
        <div className="px-4 pb-3 space-y-2">
          ...
          {activeMembers.length > 0 && (
            <div className="flex -space-x-1.5">
              {activeMembers.slice(0, 8).map(m =>
                m.profile ? (
                  <Avatar key={m.user_id} profile={m.profile} size="xs"
                    showLocalBadge className="ring-2 ring-white" />
                ) : null
              )}
            </div>
          )}
          ...
          <div className="flex gap-2 pt-1">
            {!isMember && spotsLeft > 0 && userId && (
              <Button onClick={() => joinPin.mutate(id)} loading={joinPin.isPending} size="sm" className="flex-1">
                Unirme al parche
              </Button>
            )}
            {isMember && !isCreator && (
