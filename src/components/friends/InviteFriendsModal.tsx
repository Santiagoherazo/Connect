...
interface InviteFriendsModalProps {
  pinId: string
  pinTitle: string
  currentMembers: string[] // user IDs already in the pin
  onClose: () => void
}

export function InviteFriendsModal({ pinId, pinTitle, currentMembers, onClose }: InviteFriendsModalProps) {
  const [invited, setInvited] = useState<Set<string>>(new Set())
  const { data: friends = [], isLoading } = useFriends(undefined)
  const inviteToPin = useInviteToPin()

  // Filter out friends already in the pin
  const availableFriends = friends.filter(f => !currentMembers.includes(f.id))
  ...
  const handleInvite = async (friend: Profile) => {
    try {
      ...
      // Already invited — still mark as invited
          ...
          {!isLoading && availableFriends.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-zinc-400 mt-1">
                {friends.length === 0
                  ? 'Agrega amigos desde sus perfiles'
                  : '¡El equipo está completo!'}
              </p>
            </div>
          )}

          {!isLoading && availableFriends.map(friend => {
            const isInvited = invited.has(friend.id)
            return (
              <div key={friend.id} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-zinc-50 transition-colors">
